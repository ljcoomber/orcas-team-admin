...
3. For each qualifying action:
   ...
   - Update the action with the draft ID: `mcp__nanoclaw__sheets_update_action({ action })` (see JSON below, with `metadata.draft_id` set)
...
Wheathampstead Wanderers U13 Orcas
```
