...
allowed-tools: mcp__nanoclaw__sheets_list_fixtures, mcp__nanoclaw__sheets_list_actions, mcp__nanoclaw__sheets_save_action
---
...
1. `mcp__nanoclaw__sheets_list_fixtures()` — get all fixtures
2. `mcp__nanoclaw__sheets_list_actions()` — get all existing actions (all statuses)
...
6. For each new action: `mcp__nanoclaw__sheets_save_action({ action })` (see JSON format below)
